const ensure = (v) => { if (v === undefined)
    throw Error("Undefined detected!"); return v; };
function solve_part1(input) {
    const patterns_display = input.split('\n').map(l => l.split('|'));
    let result = patterns_display.reduce((sum1, line) => sum1 + line[1].split(' ').reduce((sum2, displayed) => sum2 + ([2, 3, 4, 7].includes(displayed.length) ? 1 : 0), 0), 0);
...
function solve_part2(input) {
    ...
    const result = patterns_display.reduce((sum1, pd) => {
        ...
        const n = [];
        [n[1], n[4], n[7], n[8]] = [2, 4, 3, 7].map(count => ensure(patterns.find(d => d.length == count)));
        ...
        n[6] = ensure(patterns.find(p => tl.every(s => p.includes(s)) && bl.every(s => p.includes(s))));
        patterns = patterns.filter(p => p != n[6]);
        n[9] = ensure(patterns.find(p => r.every(s => p.includes(s)) && tl.every(s => p.includes(s))));
        patterns = patterns.filter(p => p != n[9]);
        n[5] = ensure(patterns.find(p => tl.every(s => p.includes(s))));
        patterns = patterns.filter(p => p != n[5]);
        n[2] = ensure(patterns.find(p => !r.every(s => p.includes(s))));
